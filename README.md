...
> 发现分两步、边搜边给：先按“城市＋岗位＋公司类型”**实时搜出公司队列**，再**逐家核验、搜完一家出一家**，用户随时可喊停；具体公司名单不写死，由运行时搜索得到。详见 [SKILL.md](SKILL.md) 第 1 步与 [source-strategy.md](references/source-strategy.md)。
...
- 变化字段；
- 截止判断；
- 分阶段流式发现参数（`discovery`：队列顺序、每批公司数、并发上限、雇主排序锚点）；
- 后台搜集命令；
- 状态保留和聊天清单规则。
...
两条真实限制须如实告知用户：① 系统定时只在电脑**开机**时触发，关机不补跑；② 定时任务默认只重新处理已有输入，要在无人对话时发现**全新岗位**需在 `config/project.json` 的 `collector.command` 配置后台联网搜集命令。都不满足时，退回“下次来用即自动增量刷新，或主动说‘查一下’”，不用系统任务伪装成云端推送。
