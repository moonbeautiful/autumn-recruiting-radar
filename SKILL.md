...
**提问形式（默认优先选择题）**：如果宿主客户端提供交互式选择组件（如弹窗单/多选），**优先用选择题**让用户点选这五项——毕业届次、目标岗位、招聘类型、公司类型给预设选项，城市可多选或填"全国"，让用户零打字完成配置。宿主不支持交互组件时，**退回自然语言逐项提问**，并在选项后用文字列出可选值（如"招聘类型：仅校招 / 校招+实习 / 校招+实习转正 / 全部"）。无论哪种形式，问题内容一致；不要因为退回文字就少问或多问。选择题只是更省事的输入方式，不是硬依赖。
