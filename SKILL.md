...
- **每家公司要给出该公司下所有符合用户硬条件（届次＋岗位＋城市＋招聘类型）的岗位，而不是一家只给一条**。例如用户勾了“校招+实习”，就把这家在该城市的校招岗和实习岗都列全；家内先给“最新发布 / 在招”的岗位（`in_hiring_first`），存量往后，单家至多 `max_per_company` 条；
- **每搜完一家就把该公司岗位追加回当前聊天**（`deliver_per_company`），不要等整队跑完；
- 关于数量，务必区分两个概念，**不要混淆**：
  - `first_batch_max_companies`（默认 3）是**第一波先出几家公司**，不是“总共只出 3 个岗位”；
  - `max_companies_per_run` 是单次运行最多深挖几家公司，`max_per_company` 是每家最多几条岗位。
  - 所以正常情况下，第一波 3 家公司往往就远不止 3 个岗位。**若最终只交付了三五个岗位就收尾，几乎一定是漏跑了流程，必须按下面自检纠正。**
- 出完第一波后，如 `ask_before_long_tail` 为真，**必须停下来主动询问**“要不要继续找下一批公司（含 AI 独角兽 / 垂类等长尾）”，用户说“继续”才扩，说“够了”才停并整理已出结果；**不允许自己悄悄结束或提前收尾**；
- 队列里未搜的公司留到下次继续，不丢弃。
...
1. 是否**真的先搜了公司队列**（1a），而不是随手给几个岗位就停；
2. 是否**按队列逐家深挖**，且**每家都给全了**符合硬条件的岗位（不是一家一条）；
3. 用户勾选的**每一种招聘类型**（如同时要校招和实习）是否都覆盖到，没有整类遗漏；
4. 若因 `first_batch_max_companies` 只出了第一波，是否**已停下询问用户要不要继续**，而不是把“第一波”当成“全部”。
